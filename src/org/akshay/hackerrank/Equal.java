...
/**
 * Created by aksh7576 on 10/20/15.
 */
...
public class Equal {
  ...
  /**
   * Increment array values by provided value.
   *
   * @param input
   * @param value
   * @param ignorePos
   */
  private static int[] incrementValueToArray(int[] input, int value, int ignorePos) {
  ...
  /**
   * Decrement array values by provided value.
   *
   * @param input
   * @param value
   * @param decrementFromPosition
   */
  private static int[] decrementValueFromArray(int[] input, int value, int decrementFromPosition) {
    int[] newArray = Arrays.copyOf(input, input.length);
    newArray[decrementFromPosition] -= value;
    return newArray;
  }

  /**
   * Find min number of operations required.
   *
   * @param candies
   * @return
   */
  private static int findMinOperations(int[] candies, int noOfOperations) {
    if (isEqual(candies))
      return noOfOperations;
    if (!isPositive(candies))
      return Integer.MAX_VALUE;

    int minNoOfOperations = Integer.MAX_VALUE;
    for (int i = 0; i < candies.length; i++) {
      int tempMinNoOfOperations = min(
          findMinOperations(decrementValueFromArray(candies, 1, i), noOfOperations + 1),
          findMinOperations(decrementValueFromArray(candies, 2, i), noOfOperations + 1),
          findMinOperations(decrementValueFromArray(candies, 5, i), noOfOperations + 1)
      );
      if (tempMinNoOfOperations < minNoOfOperations)
        minNoOfOperations = tempMinNoOfOperations;
    }
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < T; i++) {
      ...
      for (int j = 0; j < N; j++) {
        // read no of candies each person has
        candies[j] = sc.nextInt();
      }
      // sort
      sort(candies);
      // find min and print
      System.out.println(findMinOperations(candies, 0));
    }
  }
}
